...
%backpropagation
  ...
  h = a3;
  %Cost function calculation // I have been using y instead of y_matrix which gave me wrong values of J
  J = 1/m * sum(sum(-y_matrix.*log(h)-(1-y_matrix).*log(1-h)));

  %Regularized cost function // It didn't pass the grader at first because I was regularizing theta's biases 
  J = 1/m * sum(sum(-y_matrix.*log(h)-(1-y_matrix).*log(1-h))) + (lambda/(2*m) * ((sum(sum(Theta1(:,2:end).^2)))+(sum(sum(Theta2(:,2:end).^2)))));

  %get delta(l) 
  d3 = a3 - y_matrix; %d3: 5000x10 // I got confused using y_matrix=1 logic matrix 
  %hidden layer errors
  d2 = d3*Theta2(:,2:end).*sigmoidGradient(z2);%d2: 5000x25
  Delta1 = d2'*a1; %25x401
  Delta2 = d3'*a2;%10x26
